...
<?php
	...
	if (!isset($_GET["a"]) || !isset($_GET["b"]) || !isset($_GET["c"])) {
		...
		if (!is_numeric($_GET["a"]) || !is_numeric($_GET["b"]) || !is_numeric($_GET["c"])) {
			...
			//        Vừa xử lý nghiệ vụ (giải bài toán)
			//        vừa trình diễn
			$a = floatval($_GET["a"]);
			$b = floatval($_GET["b"]);
			$c = floatval($_GET["c"]);
			
			if ($a != 0) { // là phương trình bậc hai
				$delta = $b*$b - 4*$a*$c;
...
?>
	<!-- Mã frontend -->
    <form>
        a = <input type="text" name="a"  value="<?php echo (isset($_GET["a"]) ? $_GET["a"] : ""); ?>"/> <br/>
        b = <input type="text" name="b" value="<?php echo (isset($_GET["b"]) ? $_GET["b"] : ""); ?>"/> <br/>
        c = <input type="text" name="c" value="<?php echo (isset($_GET["c"]) ? $_GET["c"] : ""); ?>"/> <br/>
